...
class FireeyeClient:
    ...
    def make_rest_api_call(self, method, endpoint, payload=None, files=None, data=None):
        try:
            ...
            logger.debug("REST API EndPoint: {0}".format(request_url))
            logger.debug("REST API Params: {0}".format(payload))
            logger.debug("REST API Payload: {0}".format(data))
            if method == 'GET':
            ...
            msg = 'An SSL error occurred: {}'.format(e)
            logger.error(msg)
            raise ConnectorError(msg)
        except req_exceptions.ConnectionError as e:
            msg = 'A connection error occurred: {}'.format(e)
            logger.error(msg)
            raise ConnectorError(msg)
        except req_exceptions.Timeout as e:
            msg = 'The request timed out: {}'.format(e)
            logger.error(msg)
            raise ConnectorError(msg)
        except req_exceptions.RequestException as e:
            msg='There was an error while handling the request: {}'.format(e)
            logger.error(msg)
            raise ConnectorError(msg)
...
def from_cyops_download_file(iri):
    try:
        ...
        if file_name is None:
            file_name = dw_file_md['filename'] if dw_file_md['filename'] is not None else "Upload_from_the_FortiSOAR"
...
def submit_urls(config, params):
    try:
        ...
        formatted_urls = "[" + ",".join("{url}".format(url=url) for url in urls) + "]"
